...
@RestController
@RequestMapping("/api/paymentgateway-kafka")
public class PaymentgatewayKafkaResource {

    private final Logger log = LoggerFactory.getLogger(PaymentgatewayKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public PaymentgatewayKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
